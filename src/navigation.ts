...
export type PageId = 'index' | 'check-in' | 'daily-log' | 'history' | 'settings';
...
export function manuallySetPageTransition(transitionName): void {
...
export const pageTransition = (_node, config: { lifecycle: 'in' | 'out' }) => ({
  ...
  css: t => {
    ...
    if (transitionName === 'fade-push' && config.lifecycle === 'in' ||
      transitionName === 'fade-pop' && config.lifecycle === 'out') {
